  ...
  Query: {
    service1: async (parent, args, context, info) => {
      ...
      try {
        const response = await axios.get(
          "http://salchipapitas:5000/comestibles"
        );
    ...
    service2: async () => {
      try {
        ...
        const data = response.data;
        let result =
          "Hola, soy servicio2 Besitos sabor cerezas! Aquí tienes algunos besos y sus frutas asociadas: ";
    ...
    service3: async () => {
      try {
        const response = await axios.get("http://cachetadacontrucha:8000/");
        const data = response.data;
        console.log(data);
        return data;
      } catch (err) {
        console.error("Error al obtener los superheroes", err);
        return "Error al obtener los superheroes, batman :v";
      }
    },
  },
};
...
async function getContext({ req }) {
  ...
  try {
    const response = await axios.post("http://authservice:3000/verify-token", {
      token: token,
    });
